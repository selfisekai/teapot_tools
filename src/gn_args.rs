...
fn serialize_gn_arg(py: &Python, _vars: &PyDict, item: &PyAny) -> String {
    if item.is_instance(PyDict::type_object(*py)).unwrap() {
        ...
        .unwrap();
    /*
    // doesn't work, idk how to detect string
    } else if item.call_method0("_is_wrapped").is_ok() {
        return py
            .eval(
                &format!(
                    "bool({})",
...
fn generate_gn_args_contents(py: &Python, vars: &PyDict, spec: &DepsSpec) -> String {
    ...
    for arg in gclient_gn_args {
        lines.push(format!(
            "{} = {}",
            &arg,
            serialize_gn_arg(py, vars, vars.get_item(&arg).unwrap())
        ));
    }
    lines.join("\n") + "\n"
}

pub fn generate_gn_args(py: &Python, vars: &PyDict, spec: &DepsSpec, base_path: &Path) {
    ...
    fs::write(gn_args_file, generate_gn_args_contents(py, vars, spec))
